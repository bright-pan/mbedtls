...
#ifdef CONFIG_MBEDTLS_NETIF_TCPIP_LWIP
...
int mbedtls_net_accept_ex( mbedtls_net_context *bind_ctx,
                        mbedtls_net_context *client_ctx, struct sockaddr_in *sin)
{
    int ret;
    int type;
...
int mbedtls_net_recvfrom( void *ctx, void *addr, unsigned char *buf, size_t len)
{
    ...
    ret = (int) recvfrom( fd, buf, len, 0, (struct sockaddr *)&sin, &sin_len);
                                  ...
                                  uint32_t timeout )
{
    ...
    /* This call will not block */
    return ( mbedtls_net_recvfrom( ctx, addr, buf, len ) );
}
...
int mbedtls_net_sendto( void *ctx, void *addr, const unsigned char *buf, size_t len)
{
    ...
    ret = (int) sendto( fd, buf, len, 0, (struct sockaddr *)&sin, sizeof(sin));
...
#define LANGO_DBG_TAG "TIMING"
#include "lango_log.h"
// ------------------
static void mbedtls_timing_test(int argc, char *argv[]) {
    if (argc < 2) {
        LANGO_LOG_ERR("Usage: %s second", argv[0]);
        return;
    }
    unsigned long timeout_sec = atoi(argv[1]);
    struct timeval now;
    gettimeofday(&now, NULL);
    LANGO_LOG_INFO("now: %ul, %ul", now.tv_sec, now.tv_usec);
    mbedtls_net_usleep(timeout_sec*1000*1000);
    gettimeofday(&now, NULL);
    LANGO_LOG_INFO("now: %ul, %ul", now.tv_sec, now.tv_usec);
}
ALIOS_CLI_CMD_REGISTER(mbedtls_timing_test, mbedtls_timing_test, mbedtls_timing_test);
#endif //TEST
